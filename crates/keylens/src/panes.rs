...
fn gauge(name: &str, fraction: f64, text: String, width: usize, inverted: bool) -> Line<'static> {
    let style = if inverted {
        Theme::gauge_inverted(fraction)
    } else {
        Theme::gauge(fraction)
    };
    Line::from(vec![
        Span::styled(format!("  {name:<26}"), Theme::label()),
...
pub fn stats(info: &ServerInfo, width: u16) -> Vec<Line<'static>> {
    // No fields at all means the server never answered `INFO` -- some Redis-compatible
    // servers don't implement it, and managed hosts block it. A page of dashes would
    // suggest a healthy server with zero traffic, which is worse than saying nothing.
    if info.fields.is_empty() {
        return vec![
            Line::raw(""),
            Line::from(Span::styled(
                "  this server does not report INFO",
                Theme::dim(),
            )),
            Line::from(Span::styled(
                "  the key browser and lenses still work",
                Theme::dim(),
            )),
        ];
    }
    ...
    lines.push(number(
        "total commands",
        info.get_u64("total_commands_processed")
            .map(format::count)
            .unwrap_or_else(|| "-".into()),
    ));
    match info.hit_rate() {
        Some(r) => lines.push(gauge(
            "hit rate",
            r,
            format!("{:.1}%", r * 100.0),
            bar_width,
    ...
    lines.push(number(
        ...
        format!(
            "{} / {}",
            info.get_u64("keyspace_hits")
                .map(format::count)
                .unwrap_or_else(|| "-".into()),
            info.get_u64("keyspace_misses")
                .map(format::count)
                .unwrap_or_else(|| "-".into())
        ),
    ));
    ...
    lines.push(number(
        "blocked",
        info.get_u64("blocked_clients")
            .map(format::count)
            .unwrap_or_else(|| "-".into()),
    ));
    lines.push(pressure(
        "rejected connections",
        info.get_u64("rejected_connections"),
    ));
    lines.push(pressure("evicted keys", info.get_u64("evicted_keys")));
    lines.push(number(
        "expired keys",
        info.get_u64("expired_keys")
            .map(format::count)
            .unwrap_or_else(|| "-".into()),
    ));
...
pub fn slowlog(app: &App) -> Vec<Line<'static>> {
    ...
    for e in entries {
        ...
        let style = if e.duration_us >= 10_000 {
            Theme::error()
        } else {
            Theme::warn()
        };
...
pub fn cluster(app: &App) -> Vec<Line<'static>> {
    ...
    if !t.enabled {
        return vec![
            ...
            Line::from(Span::styled("  this is a standalone server", Theme::dim())),
        ];
    }

    let state_style = if t.state == "ok" {
        Theme::ok()
    } else {
        Theme::error()
    };
    ...
    for n in &t.nodes {
        ...
        let role_style = if n.master {
            Theme::accent()
        } else {
            Theme::dim()
        };
        let link_style = if n.link_state == "connected" {
            Theme::ok()
        } else {
            Theme::error()
        };
